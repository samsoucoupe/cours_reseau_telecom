...
def analyze_ethernet_header(frame):
    ...
    return (
        analyze_mac_address(eth_mac_dest),
        analyze_mac_address(eth_mac_src),
        eth_type,
    )
...
def analyze_ipv4_header(frame):
    ...
    src_ip = struct.unpack("!BBBB", ipv4_fields[8])
    src_ip = ".".join([str(i) for i in src_ip])
    dst_ip = struct.unpack("!BBBB", ipv4_fields[9])
...
def analyze_ipv6_header(frame):
    ...
    traffic_class = (ipv6_fields[0] & 0x0F) << 4 | (
            ipv6_fields[1] >> 4
    )  # 4 bits de poids faible + 4 bits de poids fort
    flow_label = (ipv6_fields[1] & 0x0F) << 16 | ipv6_fields[
        2
    ]  # 4 bits de poids faible + 16 bits de poids fort
